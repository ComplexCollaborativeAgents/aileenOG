...
class CompositeWordLesson:

    def __init__(self, is_positive, signal, description, distractors, content):
        self._composite_configurations_set = CompositeWordLesson.get_composite_configurations_set()

        self._description = description
        configuration = None
        if self._description:
            configuration = description.get("composite", None)
        if configuration:
            self._composite_configuration = configuration
        else:
            self._composite_configuration = CompositeWordLesson.randomizer.random_composite_configuration(
                self._composite_configurations_set.keys())

        self._is_positive = is_positive
        self._composite_configuration_def = self._composite_configurations_set[self._composite_configuration]

        if not is_positive:
            other_composite_configs = deepcopy(self._composite_configurations_set.keys())
            other_composite_configs.remove(self._composite_configuration)
            self._composite_configuration_negative = CompositeWordLesson.randomizer.random_composite_configuration(
                other_composite_configs)
            self._composite_configuration_def_negative = self._composite_configurations_set[self._composite_configuration_negative]
    ...
    def generate_setup(self, object_descriptions):
        ...
        objects = self._composite_configuration_def[settings.COMPOSITE_DEF_OBJECTS]
        ...
        if self._is_positive:
            self._language = LanguageGenerator.generate_language_from_template(self._scene_objects,
                                                                               self._composite_configuration_def[
                                                                                   settings.COMPOSITE_DEF_LANGUAGE_TEMPLATE])
        else:
            self._language = LanguageGenerator.generate_language_from_template(self._scene_objects,
                                                                               self._composite_configuration_def_negative[settings.COMPOSITE_DEF_LANGUAGE_TEMPLATE])

    def generate_scene(self, positions):
        logging.debug("[aileen_composite_word_lesson] :: generating a new scene for composite word learning")
    ...
    @staticmethod
    def get_composite_configurations_set():
        root_dir = os.path.dirname(os.path.abspath(__file__))
        composite_configuration_file = os.path.join(root_dir, 'resources',
                                                  settings.COMPOSITE_CONFIGURATION_FILE_NAME)
        with open(composite_configuration_file) as f:
            composite_configurations = json.load(f)
        return composite_configurations
    ...
    def administer_curriculum(world_server, agent_server):
        while True:
            raw_input("Press any key to generate the next composite word lesson...")

            lesson_object = CompositeWordLesson(is_positive=True,
                                              signal="inform",
                                              description=None,
                                              distractors=None,
                                              content=None)
    ...
    class Randomizer:
        ...
        def random_composite_configuration(self, configurations):
...
if __name__ == '__main__':
    lesson1 = CompositeWordLesson().generate_lesson()
    print(lesson1)
